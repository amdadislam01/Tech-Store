    ...
    <div className="bg-[#F8FAFC] min-h-screen py-20">
      <div className="max-w-2xl mx-auto px-4">
        <motion.div 
          ...
          className="bg-white rounded-[32px] md:rounded-[40px] p-6 sm:p-10 md:p-14 shadow-2xl shadow-gray-200/50 border border-gray-100 relative overflow-hidden"
        >
          <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-primary to-primary-dark" />
          
          <div className="flex justify-between items-start mb-12">
            <div>
              <h1 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight mb-2">Complete Payment</h1>
              <p className="text-gray-400 font-bold text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-2">
                <ShieldCheck size={14} className="text-primary" />
                Order #{id.slice(-8).toUpperCase()}
              </p>
            </div>
            <div className="text-right">
              <span className="text-gray-400 text-[10px] font-black uppercase tracking-widest block mb-1">Total</span>
              <span className="text-2xl sm:text-3xl font-black text-primary tracking-tighter">${order.totalPrice.toFixed(2)}</span>
            </div>
          </div>
          ...
          {paymentMethod === "stripe" ? (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
               <StripePayment 
                ...
                publishableKey={process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || settings?.stripePublishableKey} 
               />
            </div>
          ) : (
